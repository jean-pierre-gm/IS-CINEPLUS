...
namespace Cineplus.Services
{
    public class TicketService : ITicketService
    {
        ...
        public ActionResult MakeOrder(Guid order, string type)
        {
            ...
            if (user == null || !validType || associate == null  &&  purchaseType == BillingService.PurchaseType.Associate ) return new BadRequestResult();
            ...
            if (tickets.Any() && _billingService.VerifyPurchase(order, purchaseType,out var confirmation))
            {
                ...
                //give bonus
                if (associate != null && purchaseType != BillingService.PurchaseType.Associate)
                {
                    _associateService.AddPoints(associate, tickets.Length * 5);
